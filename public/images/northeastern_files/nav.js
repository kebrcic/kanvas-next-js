	...
	$(function () {
		...
		const main_nav = {
			...
			_init: function () {
				...
				let openBtn = $('body').hasClass('menu-open--button');
				let openHov = $('body').hasClass('menu-open--hover');

				if (openBtn) {
					main_nav.navOpenMethod = 'btn';
				} else if (openHov) {
					main_nav.navOpenMethod = 'hover';
				}
			...
			_did_click_submenu_toggle: function (e) {
				...
				if ( main_nav.navOpenMethod == 'hover' && window.innerWidth > 1100) return;
